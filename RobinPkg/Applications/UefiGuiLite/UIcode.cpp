...
public:
	...
	bool show()
	{
		...
		//erase footprint
		int x = (int)((m_start_len) * cos(m_start_angle) + m_pos_x);
		int y = (int)((m_start_len) * sin(m_start_angle) + m_pos_y);
		s_surface->draw_line(m_pos_x, m_pos_y, x, y, 0, Z_ORDER_LEVEL_0);

		m_start_len += m_len_step;
		m_start_angle += m_angle_step;

		x =(int) ((m_start_len) * cos(m_start_angle) + m_pos_x);
		y =(int) ((m_start_len) * sin(m_start_angle) + m_pos_y);
...
{
public:
	c_clock() : m_x(120), m_y(120), pi((float)(3.1415926535 / 180)) {}
	...
	bool drawSacle()
	{
		...
		drawCircle((int)m_start_radius, 0);
		m_start_radius += m_step;
		drawCircle((int)m_start_radius, GL_RGB(0, 175, 235));
		return false;
	}
	void show()
	{
	...
	void drawCircle(int r, unsigned int rgb)
	{
		...
		{
			tmpX =(int)(r * cos(angle * pi));
			tmpY =(int)(r * sin(angle * pi));
